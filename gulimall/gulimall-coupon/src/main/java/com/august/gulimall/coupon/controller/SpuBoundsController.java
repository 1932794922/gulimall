...
import com.august.gulimall.coupon.excel.SpuBoundsExcel;
...
public class SpuBoundsController {
    ...
    @ApiOperation("保存优惠价格")
    @LogOperation("保存优惠价格")
    public Result saveSpuBounds(@RequestBody SpuBoundsDTO dto){
        //效验数据
        ValidatorUtils.validateEntity(dto, AddGroup.class, DefaultGroup.class);
        spuBoundsService.save(dto);
        return new Result();
    ...
    @PostMapping("saveureduction")
    @ApiOperation("保存sku的优惠、满减等信息")
    @LogOperation("保存sku的优惠、满减等信息")
    Result saveSkuReduction(@RequestBody SkuReductionTO skuReductionTO){
        spuBoundsService.saveSkuReduction(skuReductionTO);
        return new Result();
    }
