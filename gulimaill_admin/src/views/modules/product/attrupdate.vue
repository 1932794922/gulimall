...
export default {
    ...
    methods: {
        ...
        getSpuBaseAttrs() {
            this.$http
                .get(`/product/attr/base/listforspu/${this.spuId}`)
                .then(({ data }) => {
        ...
        showBaseAttrs() {
            let _this = this
            this.$http
                .get(`/product/attrgroup/${this.catalogId}/withattr`)
                .then(({ data }) => {
                    //先对表单的baseAttrs进行初始化
                    data.data.forEach(item => {
                        let attrArray = []
        ...
        submitSpuAttrs() {
            ...
            this.$confirm('修改商品规格信息, 是否继续?', '提示', {
                ...
                .then(() => {
                    this.$http
                        .post(`/product/attr/update/${this.spuId}`, submitData)
                        .then(({ data }) => {
                            this.$message({
                                type: 'success',
                                message: '属性修改成功!'
