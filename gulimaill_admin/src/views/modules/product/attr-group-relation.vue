...
export default {
  ...
  methods: {
    ...
    batchDeleteRelation(val) {
      ...
      this.$http
        .post('/product/attrgroup/attr/relation/delete', postData)
        .then(({ data }) => {
          if (data.code == 200) {
    ...
    relationRemove(attrId) {
      ...
      this.$http
        .post('/product/attrgroup/attr/relation/delete', data)
        .then(({ data }) => {
          if (data.code == 200) {
            this.$message({ type: 'success', message: '删除成功' })
            this.init(this.attrGroupId)
    ...
    submitAddRealtion() {
      ...
      if (this.innerdataListSelections.length > 0) {
        ...
        this.$http
          .post('/product/attrgroup/attr/relation', postData)
          .then(({ data }) => {
            if (data.code == 200) {
    ...
    init(id) {
      ...
      this.visible = true
      this.$http
        .get('/product/attrgroup/' + this.attrGroupId + '/attr/relation')
        .then(({ data }) => {
    ...
    getDataList() {
      ...
      this.$http
        .get('/product/attrgroup/' + this.attrGroupId + '/noattr/relation', {
          params: {
            page: this.pageIndex,
            limit: this.pageSize,
